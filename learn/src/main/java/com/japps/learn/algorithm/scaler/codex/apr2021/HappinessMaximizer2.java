/*
* $Id: HappinessMaximizer2.java 11-Apr-2021 1:56:25 pm SubhajoyLaskar $
...
/**
 * The HappinessMaximizer2.
 *
 * @author Subhajoy Laskar
 * @version $Revision: $
 */
public class HappinessMaximizer2 {
     ...
     * @return the int
     */
    public int solve(final int A, final ArrayList<String> B) {

     if (B.stream().allMatch(group -> group.length() > A)) {
         return 0;
     }

     final List<String> happinessList = B.stream().filter(group -> group.length() <= A).map(group -> happiness(group)).collect(Collectors.toList());

     int max = 0;
     for (int i = 0; i < happinessList.size() - 1; i++) {
         for (final String element : happinessList) {
             final String gHI[] = happinessList.get(i).split(COLON);
             final String gHJ[] = element.split(COLON);

             if (gHI[0].length() + gHJ[0].length() <= A) {
                 max = Math.max(max, Integer.valueOf(gHI[1]) + Integer.valueOf(gHJ[1]));
             }
         }
     }
